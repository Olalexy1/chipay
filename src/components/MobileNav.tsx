        ...
        <SheetContent side="left" className="border-none bg-white w-[90%] h-screen">
          <Link href="/" className="cursor-pointer flex items-center gap-1 px-4">
            <Image
              ...
              alt="chiPay logo"
            />
            <h1 className="text-26 font-ibm-plex-serif font-bold text-black-1">ChiPay</h1>
          </Link>
                  ...
                  return (
                    <SheetClose asChild key={item.route}>
                      <Link href={item.route} key={item.label}
                        className={cn('mobilenav-sheet_close w-full group', { 'bg-blue-800': isActive })}
                      >
                        <Image
                          src={item.imgURL}
                          alt={item.label}
                        ...
                        />
                        <p className={cn("text-16 font-semibold text-black-2 group-hover:text-white", { "text-white": isActive })}>
                          {item.label}
                        </p>
                      </Link>
                    </SheetClose>
